...
from fixed_dzh_mcp_server import FixedDZHDeepSeekMCPServer

async def test_mcp_server():
    """测试MCP服务器"""
    print("🧪 测试修复后的DZH MCP服务器")
    print("=" * 50)
    ...
    success_count = sum(1 for r in responses if "result" in r and
                        r["result"]["content"][0]["text"].strip().startswith('"success": true'))
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "debug":
        ...
        print("🔧 DZH MCP服务器测试工具")
        print("用法: python test_fixed_dzh_mcp.py debug")
        print("       运行调试模式测试")
